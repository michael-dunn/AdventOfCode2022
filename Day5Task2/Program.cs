public class Program
{
    public static void Main(string[] args)
    {
        ...
        var movementStack = new Stack<string>();
        foreach (var instruction in instructionData)
        {
            for (int i = 0; i < instruction[0]; i++)
            {
                movementStack.Push(stacks[instruction[1] - 1].Pop());
            }

            for (int i = 0; i < instruction[0]; i++)
            {
                stacks[instruction[2] - 1].Push(movementStack.Pop());
            }

            movementStack.Clear();
        }

        Console.WriteLine(string.Join("", stacks.Select(s => s.Peek()[1])));
    }
}